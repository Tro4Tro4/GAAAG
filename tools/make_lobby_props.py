...
def notice() -> PixelCanvas:
    """The regulations, 44x32. The lines only suggest text — the words the
    player reads come from the localised look_text, not from the sprite."""
    c = PixelCanvas(44, 32)
    p = ramp(PAPER)
    c.rect(0, 0, 43, 31, OUTLINE)
    c.rect(1, 1, 42, 30, p[1])
    c.rect(1, 1, 42, 3, p[2])                       # lit top edge
    c.rect(1, 29, 42, 30, p[0])
    for i, y in enumerate(range(7, 29, 4)):
        c.rect(4, y, 39 - (i % 3) * 7, y, p[0])     # ragged lines of small print
    return c


def rack() -> PixelCanvas:
    """The form holder, 28x44: two trays, the upper one full and the lower empty.
    The joke is in the look_text; the sprite only has to make it visible."""
    c = PixelCanvas(28, 44)
    m = ramp(METAL)
    p = ramp(PAPER)
    c.rect(0, 0, 27, 43, OUTLINE)
    c.rect(1, 1, 26, 42, m[1])
    c.rect(1, 1, 26, 2, m[2])
    c.rect(3, 4, 24, 19, m[0])                      # upper tray, with forms
    c.rect(4, 5, 23, 18, p[1])
    c.rect(4, 5, 23, 6, p[2])
    c.rect(3, 24, 24, 39, m[0])                     # lower tray, empty
    c.rect(4, 25, 23, 38, m[0])
    return c


def chairs() -> PixelCanvas:
    """Four chairs in a row, 68x22, seen from a little above. The node goes where
    they touch the floor, so the sprite is drawn with its feet on the last row."""
    c = PixelCanvas(68, 22)
    w = ramp(WOOD)
    for i in range(4):
        x = i * 17
        c.rect(x + 2, 1, x + 13, 8, w[0])           # back
        c.rect(x + 3, 2, x + 12, 7, w[1])
        c.rect(x + 1, 9, x + 14, 13, w[1])          # seat
        c.rect(x + 1, 9, x + 14, 10, w[2])
        c.rect(x + 2, 14, x + 3, 21, w[0])          # legs
        c.rect(x + 12, 14, x + 13, 21, w[1])
        c.rect(x + 5, 14, x + 6, 19, w[0])
        c.rect(x + 10, 14, x + 11, 19, w[0])
    return c
